...
PROFILE  =  "Lillith"
STYLE    =  "expressive"  # warmer tone, slight creativity bump
...
def process_text(text:str) -> Dict[str, Any]:
    ...
    # expressive tweaks
    out["tone"]["warmth"]  =  min(1.0, out["tone"]["warmth"] + 0.05)
    out["abstract"]["creativity"]  =  min(1.0, out["abstract"]["creativity"] + 0.05)
    return out

def speak_summary(analysis: Dict[str, Any]) -> bool:
    t  =  analysis.get("tone",{})
    msg  =  f"Lillith ready. tone {t.get('positivity',0):.2f}/warmth {t.get('warmth',0):.2f}. (voice: {backend()})"
