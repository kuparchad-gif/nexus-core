...
import uuid
import subprocess
from soulprint import verify_soulprint, generate_soulprint, quarantine_node
...
def main():
    """Lightweight node loop with soulprint verification."""
    node_id = os.getenv("NODE_ID", f"node-{uuid.uuid4().hex[:8]}")
    node_type = os.getenv("NODE_TYPE", "node")
    ...
    c = consul.Consul(host="nexus-consul.us-east-1.hashicorp.cloud", token="d2387b10-53d8-860f-2a31-7ddde4f7ca90")
    port = {"anynode": 8081, "soulsync": 8082, "nexpulse": 8083, "chaosshield": 8084}.get(node_type, 5000)
    ...
    c.agent.service.register(
        ...
        meta={"type": node_type, "project": project, "env": env}
    )
    ...
    while True:
        ...
        c.agent.service.register(
            name=f"lilith-{node_id}", service_id=node_id, address=f"{node_id}.local", port=port,
            meta={"type": node_type, "cpu": str(metrics["cpu"]), "mem": str(metrics["mem"]), "project": project, "env": env}
        )
