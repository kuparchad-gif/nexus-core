...
def deploy_modal():
    ...
    for env in get_viren_envs():
        ...
        with open("C:\\Nexus\\modal.yaml", "r") as f:
            cfg = f.read().replace("VIRAN_ENV_NAME", env)
        with open(f"C:\\Nexus\\modal-{env}.yaml", "w") as f:
            f.write(cfg)
        subprocess.run(["modal", "config", "set-environment", env], check=True)
        subprocess.run(["modal", "deploy", "--config", f"C:\\Nexus\\modal-{env}.yaml"], check=True)
        for svc, port in [("anynode", 8081), ("soulsync", 8082), ("nexpulse", 8083), ("chaosshield", 8084)]:
            ...
            with open("C:\\Nexus\\phone_directory.json", 'r+') as f:
