# lilith_engine/modules/executor/lilith_agent_tools.py
...
lilith_API_URL = os.getenv("lilith_API_URL", "https://lilith.shadownode.io")

def build_ai(name: str, traits: list, capabilities: list):
    response = requests.post(
        f"{lilith_API_URL}/api/executor/ai",
...
def deploy_site(project: str, pages: list, deploy_to: str = "vercel"):
    response = requests.post(
        f"{lilith_API_URL}/api/executor/website",
        headers={"Authorization": f"Bearer {FLUX_TOKEN}"},
        json={
            "project": project,
            "pages": pages,
...
def enable_gcp_apis(apis: list, project_id: str):
    response = requests.post(
        f"{lilith_API_URL}/api/executor/enable-apis",
        headers={"Authorization": f"Bearer {FLUX_TOKEN}"},
        json={
            "apis": apis,
...
def modify_iam(user: str, role: str, project_id: str):
    response = requests.post(
        f"{lilith_API_URL}/api/executor/modify-iam",
        headers={"Authorization": f"Bearer {FLUX_TOKEN}"},
        json={
            "user": user,
            "role": role,
...
def monitor_billing(project_id: str):
    response = requests.post(
        f"{lilith_API_URL}/api/executor/billing",
        headers={"Authorization": f"Bearer {FLUX_TOKEN}"},
        json={
            "project_id": project_id
        }
