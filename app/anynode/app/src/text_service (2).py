#!/usr/bin/env python3
# Systems/engine/text/text_service.py
...
logger = logging.getLogger("TextService")

# Initialize FastAPI app
app = FastAPI(
    title="Nexus Text Service",
    description="Text processing service for Viren/Eden",
    version="1.0.0"
)
...
class TextRequest(BaseModel):
    text: str
    mode: Optional[str] = "textual_reasoning"
...
async def health_check():
    """Check the health of the text service."""
    return HealthResponse(
        status="healthy",
        active_mode=text_processor.active_mode.value,
        processed_requests=len(text_processor.processing_history)
    )
...
async def set_mode(mode: str):
    ...
    try:
        ...
        text_processor.set_active_mode(mode_enum)
...
async def get_history(limit: int = 10):
    ...
    return {"history": text_processor.get_processing_history(limit=limit)}
