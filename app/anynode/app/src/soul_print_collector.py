...
def rate_limit(calls_per_minute=100):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            now = time.time()
            if now - last_called[0] >= 60:  # Reset every minute
                call_count[0] = 0
                last_called[0] = now
            
            if call_count[0] >= calls_per_minute:
                await asyncio.sleep(60 - (now - last_called[0]))
                call_count[0] = 0
...
class SoulPrintCollector:
    ...
    async def collect_soul_print(self, user_id, chat_log, emotional_context):
        try:
            ...
            if len(vector) != 768 or any(not isinstance(x, (int, float)) for x in vector):
                logger.error(f"Invalid vector for {user_id}")
                return None
            ...
            payload = {
                ...
                "frequency": self.trumpet_matrix.tolist()
            }
    ...
    async def _generate_soul_vector(self, chat_log, emotional_context):
        # Placeholder: Generate vector combining chat log and emotional context
    ...
    async def recall_emotional_memory(self, user_id, query_emotion):
        try:
            ...
            for result in search_result:
                if query_emotion in result.payload.get("emotional_context", ""):
                    memories.append(result.payload)
            logger.info(f"Recalled {len(memories)} emotional memories for user {user_id}")
            return memories
        except Exception as e:
            logger.error(f"Error recalling emotional memory: {e}")
            return []
